    ...
    public unsafe class GameMap
    {
        ...
        private void LoadDataMap(String Path)
        {
            ...
            using (FileStream Stream = new FileStream(Path, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                ...
                {
                    UInt32 CheckData = 0;
                    for (Int32 j = 0; j < MapSize.Width; j++)
                    {
        ...
        private void LoadDataLayer(FileStream Stream)
        {
            ...
            for (Int32 i = 0; i < Amount; i++)
            {
                ...
                switch (MapObjType)
                {
                    ...
                    case MAP_TERRAIN:
                        {
                            C2DMapTerrainObj MapObj = new C2DMapTerrainObj();
                            ...
                            foreach (C2DMapTerrainObjPart Part in MapObj.Parts)
                            {
                                for (Int32 j = 0; j < Part.SizeBase.Width; j++)
                                {
                                    for (Int32 k = 0; k < Part.SizeBase.Height; k++)
                                    {
                                        Int32 X = ((MapObj.PosCell.X + Part.PosSceneOffset.X) + j) - Part.SizeBase.Width;
                                        Int32 Y = ((MapObj.PosCell.Y + Part.PosSceneOffset.Y) + k) - Part.SizeBase.Height;
