    ...
    public unsafe class WDF
    {
        ...
        public void Close()
        {
            ...
            if (Entries != null)
            {
                lock (Entries)
                {
                    foreach (IntPtr pEntry in Entries.Values)
        ...
        public Boolean DelEntry(UInt32 ID)
        {
            ...
            lock (Entries)
            {
                ...
                pHeader->Number--;
                pHeader->Offset -= pEntry->Size;
                using (FileStream Stream = new FileStream(Filename, FileMode.Open, FileAccess.Write, FileShare.Read))
                {
                    Kernel.memcpy(Buffer, pHeader, sizeof(Header));
            ...
            using (FileStream FStream = new FileStream(Destination, FileMode.Create, FileAccess.Write, FileShare.Read))
            {
                ...
                using (StreamWriter Writer = new StreamWriter(Destination.Replace(".wdf", ".lst"), false, Encoding.Default))
                {
                    ...
                    for (Int32 i = 0; i < pHeader->Number; i++)
                    {
                        ...
                        UInt32 UID = String2ID(RelativePath);
